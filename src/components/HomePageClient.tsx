...
function WeatherSkeleton() {
  return (
    <div className="animate-pulse space-y-4" aria-label="날씨 정보 로딩 중" role="status">
      ...
      <div className="rounded-2xl bg-white/10 p-4">
        ...
        <div className="flex gap-3">
          {[0, 1, 2, 3, 4].map((i) => (
            <div key={i} className="h-28 w-20 bg-white/20 rounded-2xl flex-shrink-0" />
          ))}
        </div>
      </div>
    </div>
  ...
  return (
    ...
    >
      ...
      <button
        type="button"
        onClick={onRetry}
        className="px-5 py-2 rounded-full bg-white/20 hover:bg-white/30 active:bg-white/40 text-white text-sm font-medium transition-colors focus-visible:outline focus-visible:outline-2 focus-visible:outline-white"
      >
        다시 시도
      </button>
    </div>
...
function HomePageContent() {
  ...
  const realWeatherState = useWeather(isDemo ? NaN : lat, isDemo ? NaN : lon);
      ...
      <div className="relative z-10 min-h-screen flex flex-col items-center px-4 py-8">
        ...
        {isDemo && (
          <div className="w-full max-w-md mb-2 px-3 py-1.5 rounded-full bg-yellow-400/20 border border-yellow-300/30 text-yellow-100 text-xs text-center">
            샘플 데이터 미리보기 — API 키 설정 후 실제 날씨를 확인하세요
          </div>
        )}
